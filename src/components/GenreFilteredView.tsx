...
export function GenreFilteredView({ query }: { query: string }) {
    ...
    return (
        <div className="space-y-12 animate-in fade-in slide-in-from-bottom-8 duration-700 min-h-[70vh]">
            ...
            <div className="relative p-8 md:p-16 rounded-[2rem] md:rounded-[3rem] overflow-hidden group border border-white/5 bg-gradient-to-br from-white/5 to-transparent">
                <div className="absolute inset-0 bg-[#8B5CF6]/10 blur-[100px] -z-10 animate-pulse"></div>
                <div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-6 md:gap-8">
                    <div className="space-y-3 md:space-y-4">
                        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#8B5CF6]/20 border border-[#8B5CF6]/20 text-[#8B5CF6] text-[10px] font-black uppercase tracking-widest">
                            Curated Mix
                        </div>
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6 md:gap-8">
                    {tracks.map((track, i) => (
                        ...
                        >
                            <div className="relative aspect-square overflow-hidden rounded-2xl md:rounded-[2rem] bg-zinc-900 border border-white/5 shadow-xl transition-all duration-500 group-hover:-translate-y-2 group-hover:shadow-2xl group-hover:shadow-[#8B5CF6]/20">
                                <Image
                            ...
                            <div className="px-2">
                                <h4 className="font-bold text-white truncate text-base group-hover:text-[#8B5CF6] transition-colors">{track.title}</h4>
                    ...
                    {tracks.length === 0 && !loading && (
                        <div className="col-span-full text-center py-40 bg-white/5 rounded-[3rem] border border-dashed border-white/10">
                            ...
                            <button onClick={() => window.location.reload()} className="text-[#8B5CF6] mt-4 hover:underline">Try another genre</button>
                        </div>
                    )}
                </div>
            )}
